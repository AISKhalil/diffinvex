...
quiet(library(arm)) # or conflict with Matrix library
quiet(library("biomaRt"), all = T)
quiet(library("rtracklayer"), all = T)
packages <- c("GenomicRanges","plyr", "dplyr", "reshape", "data.table","stringr")
...
read_genomic_tracks = function(reference_genome, 
  ...
  if(reference_genome == "hg19"){
      MicrosatellitesTracks  <- paste0(toolDirectory,"/references/GenomicTracks/Repeats/hg19_repeats_6ntsOrMore.bed")
  ...
  } else if(reference_genome == "hg38"){ #To-be-updated (not working)
      MicrosatellitesTracks  <- paste0(toolDirectory,"/references/GenomicTracks/Repeats/hg19_repeats_6ntsOrMore.bed")
      CTCF_Tracks            <- paste0(toolDirectory,"/references/GenomicTracks/CTCF_regions/CTCF_Cohesion_peaks_RenLab_Motifs_hg19.bed")
